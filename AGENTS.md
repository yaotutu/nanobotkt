...
3. NanobotKT 是 Android 手机客户端，唯一允许连接的服务端入口是 `http://192.168.55.201:8765/`。HTTP、Bootstrap 与 WebSocket 都必须使用该主机和 `8765` 端口；WebSocket 只能在此入口上使用服务端下发的路径与令牌。除非用户明确指定新的服务端地址，禁止使用或替换为 `127.0.0.1`、`localhost`、`10.0.2.2`、`18790` 或任何其他主机/端口，禁止通过 `adb reverse` 端口映射绕过此约束，也禁止把服务端内部监听地址当成客户端连接地址。
