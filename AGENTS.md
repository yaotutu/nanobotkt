...
3. NanobotKT 是 Android 手机客户端，`http://192.168.55.147:8765/` 是默认开发服务端地址；正式产品允许用户在认证页或 Settings 中主动填写并切换 Gateway 基础地址。HTTP、Bootstrap 与 WebSocket 必须始终使用同一个用户已选择的基础地址，WebSocket 只能在该入口上使用 Bootstrap 下发的路径与令牌。禁止自动把服务端内部监听地址当成客户端入口，禁止通过 `adb reverse`、`127.0.0.1`、`localhost`、`10.0.2.2` 或其他隐式替换绕过用户配置；开发或测试代码也不得擅自覆盖用户已选择的主机和端口。
