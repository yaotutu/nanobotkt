...
internal fun OverviewPage(
    ...
    SettingsGroup(stringResource(R.string.settings_group_ai_capabilities)) {
        SettingsRow(
            ...
            showBrandLogos = showBrandLogos,
            onClick = { onSectionChange(SETTINGS_SECTION_WEB) },
        )
        CardDivider()
        SettingsRow(
            ...
            showBrandLogos = showBrandLogos,
            onClick = { onSectionChange(SETTINGS_SECTION_IMAGE) },
        )
        CardDivider()
        SettingsRow(
            ...
            showBrandLogos = showBrandLogos,
            onClick = { onSectionChange(SETTINGS_SECTION_VOICE) },
        )
    }
    ...
    SettingsGroup(stringResource(R.string.settings_group_system_security)) {
        ...
        SettingsRow(
            ...
            title = stringResource(R.string.settings_gateway),
            subtitle =
            ...
            onClick = { onSectionChange(SETTINGS_SECTION_SYSTEM) },
        )
        CardDivider()
        SettingsRow(
...
internal fun TokenUsageHeatmapCard(usage: SettingsUsage?) {
    ...
    val monthFormat = remember(displayLocale) { SimpleDateFormat("MMM", displayLocale) }
    val days =
        remember(endStart) {
            List(371) { index ->
                Calendar.getInstance().apply {
